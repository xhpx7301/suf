...
```text
SUF 1.1.2 - Server UFW & SSH Fortress
...
| `3` 查看实际监听端口 | 显示正在监听的服务，不代表该端口已经被 UFW 放行。仅本机地址 `127.0.0.1` 或 `[::1]` 的端口通常无需对公网开放。 |
| `4` 安装 UFW | 只安装软件包，不会自动开启防火墙。 |
| `5` 引导式初始配置 | 设置“拒绝入站、允许出站”，放行当前 SSH 端口，按需放行 HTTP/HTTPS 和额外端口，确认后启用 UFW。首次配置推荐使用。 |
| `6` 放行端口 | 允许所有来源访问指定 TCP 或 UDP 端口。只用于确实需要公开的服务。 |
| `7` 仅允许指定 IP/CIDR 访问端口 | 仅向一个 IP 或网段开放端口；SUF 会检查并要求删除同端口的广泛放行规则。 |
| `8` 为端口添加连接限速 | 对 TCP 新连接施加 UFW 限速，适合 SSH 等管理端口；不能替代密钥认证或 Fail2ban。 |
| `9` 删除编号规则 | 按选项 `2` 显示的编号删除规则。删除后编号会重新排列。 |
| `10` 启用 UFW | 对已有规则启用防火墙，并优先为当前 SSH 来源保留管理入口。 |
| `11` 重新加载规则 | 重新读取已保存的 UFW 规则，通常不需要重启服务器。 |
| `12` 关闭 UFW | 停止执行防火墙规则，所有监听端口失去这层保护。 |
| `13` 卸载 UFW | 移除软件包但保留配置文件；以后重装仍可能读取旧规则。 |
